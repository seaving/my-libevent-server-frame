		...
		char *response_buf, int bufsize)
{
	...
	return protocol_task_pack_brush_response(&resp, response_buf, bufsize);
}
...
static inline bool _protocol_data_parse(
		evexecutor_t executor, int cmd, 
		const struct json_object *data_obj, 
		char *response_buf, int bufsize)
{
	if (data_obj == NULL)
	{
	...
	switch (cmd)
	{
		case E_PROTOCOL_CMD_TASK_REQUEST:
		{
			...
			return _protocol_task_request_parse(executor, data_obj, response_buf, bufsize);
		}
		case E_PROTOCOL_CMD_TASK_RESPONSE:
		{
			LOG_TRACE_NORMAL("CMD = E_PROTOCOL_CMD_TASK_RESPONSE \n");
			return _protocol_task_response_parse(executor, data_obj, response_buf, bufsize);
		}
		case E_PROTOCOL_CMD_PROBE:
		{
			LOG_TRACE_NORMAL("CMD = E_PROTOCOL_CMD_PROBE \n");
			return _protocol_probe_parse(executor, data_obj, response_buf, bufsize);
		}
		case E_PROTOCOL_CMD_BECON:
		{
			LOG_TRACE_NORMAL("CMD = E_PROTOCOL_CMD_BECON \n");
			return _protocol_becon_parse(executor, data_obj, response_buf, bufsize);
		}
		default:
		{
...
bool protocol_parse(evexecutor_t executor, httpRequest_t *httpRequest, char *response_buf, int bufsize)
{
	...
	ret = _protocol_data_parse(executor, cmd, data_obj, response_buf, bufsize);
	json_object_put(json_obj);
